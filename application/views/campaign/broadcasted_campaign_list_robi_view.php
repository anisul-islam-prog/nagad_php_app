<h1 class="heading"><?php if (1) echo ''; ?> Broadcasted Robi Campaign List</h1>
...
<div class="col-sm-12">
  ...
  <div class="row-fluid">
            ...
            <div class="span12">
                 <table class="table table-bordered table-striped" id="broadcasted_robi_table" style="margin-bottom: 0;">
                    <thead>
                    <tr>
                        <th>Campaign ID</th>
						<th>Masking Name</th>
                    ...
                    <tr> 
					    ...
					    <td><a href="<?php echo site_url('dnd_controllers/campaign/download_delivered_history_when_stopped_robi/'.$key['CAMPAIGN_ID']); ?>"  <?php if($key['IS_PAUSED']!=2) {?>disabled<?php } ?> class="btn btn-success">Download</a></td>	    					
						...
						<td width="200" class="center"><a href="<?php echo site_url('dnd_controllers/campaign/resume_or_pause_robi/'.$key['CAMPAIGN_ID'].'/'.$key['IS_PAUSED']); ?>" <?php if($key['IS_PAUSED']==2 || (($key['ALL_COUNT']*1.0)/($key['GENERATED_BASE']*1.0))*100>98 ) {?>disabled<?php } ?> class="btn <?php if($key['IS_PAUSED']==0) {?>btn-warning<?php }else{?>btn-success<?php }?>"><?php if($key['IS_PAUSED']==1) {?>Resume<?php }else{?>Pause<?php }?></a>			
						<a href="<?php echo site_url('dnd_controllers/campaign/stop_campaign_robi/'.$key['CAMPAIGN_ID']); ?>"  <?php if($key['IS_PAUSED']==2 || (($key['ALL_COUNT']*1.0)/($key['GENERATED_BASE']*1.0))*100>98) {?>disabled<?php } ?> class="btn btn-danger">Stop</a></td> 
...
<script>
 $(document).ready(function() {
        $('#broadcasted_robi_table').DataTable(
		{
			 scrollY:        "330px",
        scrollX:        "300px;",
        scrollCollapse: true,
